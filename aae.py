  ...
  #      self.sess = tf.Session()
   ...
   #     return tf.reduce_mean(tf.nn.sigmoid_cross_entropy_with_logits(logits= x, labels = y))
    ...
    def generate_con_image(self):
        ...
        for i in range(self.conf.n_class):
            ...
            for k in range(imgs.shape[0]):
                imgs_folder = os.path.join(self.conf.working_directory, 'imgs_con_pixel', str(i))
    ...
    def evaluate(self, data):        
     #   data = data_reader()
        pbar = ProgressBar()
        imgs_original_folder = os.path.join(self.conf.working_directory, 'imgs_original_pixel')
        if not os.path.exists(imgs_original_folder):
            os.makedirs(imgs_original_folder)
        imgs_test_folder = os.path.join(self.conf.working_directory, 'imgs_test_pixel')
    ...
    def generate_and_save(self):
        ...
        for k in range(imgs.shape[0]):
            imgs_folder = os.path.join(self.conf.working_directory, 'imgs_pixel')
            if not os.path.exists(imgs_folder):
                os.makedirs(imgs_folder)      
